...
/// Число отсчётов осциллографа — НЕ произвольное «поплотнее, чтобы было
/// гладко», а ровно столько, сколько значений успеет уйти на устройство за
/// это окно (`hid::worker::SEND_INTERVAL_S`). Иначе график врёт в самую
/// важную сторону: раньше он брал 240 точек на 2 с (шаг 8.3 мс) и рисовал
/// идеальную математику формы, тогда как мотор получает значение раз в
/// 20 мс. На «Пульсации» 4 Гц это расходилось так: период 250 мс не делится
/// нацело на такт 20 мс (12.5 такта), поэтому реально импульсы чередуются
/// 140 и 120 мс — а на графике все восемь выглядели одинаковыми по 125 мс.
/// Теперь окно дискретизируется тем же шагом, что и выход, и такая
/// неравномерность видна на графике ровно там, где она есть на моторе.
const OSCILLOSCOPE_SAMPLES: usize =
    (OSCILLOSCOPE_WINDOW_S / crate::hid::protocol::SEND_INTERVAL_S + 0.5) as usize;
// `+ 0.5` выше — округление, а не усечение: `as usize` отбрасывает дробную
// часть, а деление двоичных float даёт то 100.0, то 99.999... в зависимости
// от конкретной пары чисел. На нынешних 2.0 / 0.020 выходит ровно 100.0, но
// стоит поменять окно или период отправки — и молча получилось бы на один
// отсчёт меньше. Ассерт ловит вырожденный случай на этапе компиляции.
const _: () = assert!(OSCILLOSCOPE_SAMPLES > 1);
