...
//! этот модуль — часть основного приложения, всегда опрашивает
//! localhost:8111 (см. `worker::wt_worker`) и шлёт вибрацию через тот же
//! `hid_worker`, что и MSFS (см. `crate::sim::sim_worker`) — какой из двух
//! конвейеров реально владеет HID-каналом в моменте, решает
