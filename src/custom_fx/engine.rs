...
//!
//! Второй движок рядом со встроенным `RumbleEngine` (см. `crate::rumble`):
//! оба считаются КАЖДЫЙ тик, их выходы сводятся `.max()` по каждому мотору,
//! а столкновения на одном источнике телеметрии разруливает вытеснение
//! встроенного эффекта (см. `super::overrides`). Многие приёмы намеренно
//! скопированы оттуда и из `crate::wt_link::rumble` (см. комментарии по
//! месту) — задача явно запрещала трогать эти файлы, поэтому
//! переиспользование только через копирование логики, не через shared-код.
...
/// пустая/отсутствующая подстрока = совпадает с любым бортом). `pub(crate)`,
/// а не приватная: переиспользуется `custom_fx::overrides::overridden_builtins`
/// — таблица вытеснения обязана фильтровать по борту той же логикой, что и
/// сам движок, иначе список вытесненных встроенных эффектов разошёлся бы с
/// тем, что реально считает `CustomFxEngine::step`.
pub(crate) fn aircraft_matches(pattern: &Option<String>, aircraft: &str) -> bool {
    match pattern {
        None => true,
