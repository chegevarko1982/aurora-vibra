...
impl RrefClient {
    ...
    pub fn connect() -> std::io::Result<Self> {
        ...
        // Полностью неблокирующий сокет (не просто короткий таймаут): тик
        // движка (см. xp_link::worker) идёт на собственном ровном таймере
        // 20 мс, drain() внутри него обязан вычерпать всё, что накопилось, и
        // вернуться немедленно, а не ждать recv() до истечения таймаута —
        // раньше здесь стоял set_read_timeout(100мс), из-за чего drain()
        // мог реально просидеть в recv() до 100 мс на каждом вызове (когда
        // все накопленные пакеты уже разобраны, а следующий ещё не пришёл),
        // что сжирало бы половину бюджета 20-мс тика.
