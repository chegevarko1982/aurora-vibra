...
fn main() {
    ...
    let lib = unsafe {
        Library::new("lib/SimConnect.dll").or_else(|_| Library::new("SimConnect.dll"))
    }
    ...
    let add_to_def_fn: PfnSimConnectAddToDataDefinition =
        unsafe { *lib.get(b"SimConnect_AddToDataDefinition\0").unwrap() };
    let req_data_fn: PfnSimConnectRequestDataOnSimObject =
        unsafe { *lib.get(b"SimConnect_RequestDataOnSimObject\0").unwrap() };
    let next_dispatch_fn: PfnSimConnectGetNextDispatch =
        unsafe { *lib.get(b"SimConnect_GetNextDispatch\0").unwrap() };
                ...
                if (*p_recv).dw_id == SIMCONNECT_RECV_ID_SIMOBJECT_DATA {
                    ...
                    if sod.dw_request_id == req_id {
                        ...
                        if last_print.elapsed() >= Duration::from_millis(100) {
                            print!(
                                "\rHandle(Pos): {:.3} | Handle(%): {:3.0}% | Left(%): {:3.0}% | Right(%): {:3.0}% | Spoilers(Pos): {:.3}",
