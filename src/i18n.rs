...
https://yoomoney.ru/to/410011348629282",
    ...
    chk_record_wt_session: "Record WT session (debug)",
    hover_record_wt_session: "Writes raw /state + /indicators telemetry from War Thunder to wt_probe_sessions/session_<timestamp>.jsonl next to the app, same format wt_probe uses — for diagnosing missing/incorrect effects on specific aircraft. Off by default, not saved between restarts.",
    ...
    hover_wt_overspeed: "Vibration on both throttle motors as indicated airspeed approaches the aircraft's Vne (never-exceed speed) — builds linearly over the last 10 km/h below the threshold, then holds at full power once Vne is reached or exceeded. Thresholds come from a ~1300-aircraft table sourced from the War Thunder wiki; aircraft missing from that table stay silent rather than guessing.",
...
https://yoomoney.ru/to/410011348629282",
    ...
    chk_record_wt_session: "Записывать сессию WT (отладка)",
    hover_record_wt_session: "Пишет сырую телеметрию /state + /indicators War Thunder в wt_probe_sessions/session_<timestamp>.jsonl рядом с программой, в том же формате, что и wt_probe — для диагностики отсутствующих/неверных эффектов на конкретных самолётах. По умолчанию выключено, между перезапусками не сохраняется.",
    ...
    name_wt_overspeed: "Overspeed (Vne)",
    hover_wt_overspeed: "Вибрация на оба мотора РУД по мере приближения приборной скорости к Vne (предельно допустимой скорости борта) — нарастает линейно на последних 10 км/ч перед порогом, затем держится на полной мощности на/за порогом. Пороги — из таблицы на ~1300 бортов с wiki War Thunder; борта вне таблицы молчат, а не гадают.",
