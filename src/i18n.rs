...
pub struct Strings {
    ...
    /// Подпись у ползунка частоты: сколько тактов отправки на устройство
    /// укладывается в один период формы. Точное число, без оценок и
    /// предупреждений — целое значит «все импульсы одной длины», дробное
    /// значит «длины чередуются», вывод делает пользователь.
    pub lbl_fx_ticks_per_period: &'static str,
    pub hover_fx_ticks_per_period: &'static str,
    ...
    /// Задача A: подпись под осциллографом шага 4 — он всегда рисует форму
    /// "на пике" кривой (не текущее значение телеметрии), эта строка не даёт
    /// принять график за живой сигнал. См. `engine::shape_waveform`.
    pub lbl_fx_oscilloscope_hint: &'static str,
...
https://yoomoney.ru/to/410011348629282",
    ...
    lbl_fx_freq: "Rate",
    hover_fx_freq: "Capped at 6.5 Hz: the device is fed 50 times a second, faster pulses are lost between ticks.",
    lbl_fx_ticks_per_period: "ticks/period",
    hover_fx_ticks_per_period: "One tick is 20 ms — the interval at which the device is fed. A whole number means every pulse comes out exactly the same length. A fractional one means lengths alternate: 4 Hz is 12.5 ticks, so pulses run 140/120 ms. Nothing can round that away, it is the send rate itself.",
    ...
    lbl_fx_oscilloscope_hint: "Shape at full response — how the effect looks at the peak of its curve, not the current telemetry value.",
    heading_fx_preview: "Preview",
    btn_fx_play: "Start on device",
    btn_fx_stop: "Stop",
...
https://yoomoney.ru/to/410011348629282",
    ...
    shape_sawtooth: "Пила",
    lbl_fx_freq: "Частота",
    hover_fx_freq: "Ограничена 6.5 Гц: на устройство мы шлём 50 раз в секунду, более частые импульсы теряются между тактами.",
    lbl_fx_ticks_per_period: "тактов на период",
    hover_fx_ticks_per_period: "Один такт — 20 мс, с таким интервалом идёт отправка на устройство. Целое число означает, что все импульсы выходят ровно одной длины. Дробное — что длины чередуются: 4 Гц это 12.5 такта, импульсы идут 140/120 мс. Округлить это нельзя, так устроена сама частота отправки.",
    ...
    lbl_fx_oscilloscope_hint: "Форма при полном отклике — как эффект выглядит на пике своей кривой, а не при текущем значении телеметрии.",
    heading_fx_preview: "Предпросмотр",
    btn_fx_play: "Запустить на устройстве",
    btn_fx_stop: "Стоп",
