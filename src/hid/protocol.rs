/// Период отправки интенсивности на устройство, в секундах — свойство
/// канала до железа, а не деталь реализации воркера, поэтому живёт здесь, в
/// платформонезависимом модуле: `hid::worker` собирается только под Windows
/// (см. `hid/mod.rs`), а осциллограф формы в редакторе эффектов
/// (`ui::effects_editor`) обязан дискретизировать окно ровно этим шагом на
/// любой платформе — иначе он рисует идеальную математику формы вместо той
/// последовательности значений, которую реально получит мотор.
///
/// По теореме Найквиста частота дискретизации СИГНАЛА равна половине
/// частоты отправки: 50 Гц отправки -> 25 Гц. При потолке частоты формы
/// `custom_fx::model::MAX_SHAPE_FREQ_HZ` = 6.5 Гц это ~7.7 отсчёта на период.
pub const SEND_INTERVAL_S: f64 = 0.020;
